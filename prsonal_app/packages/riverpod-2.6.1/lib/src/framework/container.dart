...
// TODO found "Future already completed error" after adding family parameter
  ...
  void updateOverrides(List<Override> overrides) {
    ...
    // Patched: allow adding new overrides dynamically (e.g. from conditional
    // test helpers that change the override list length).

    for (final override in overrides) {
      if (override is ProviderOverride) {
        if (!_overrideForProvider.containsKey(override._origin)) {
          // New override: register it as if it were present from the start.
          _overrideForProvider[override._origin] = override._override;
          _stateReaders[override._origin] = _StateReader(
            origin: override._origin,
            override: override._override,
        ...
        if (!_overrideForFamily.containsKey(override.overriddenFamily)) {
          _overrideForFamily[override.overriddenFamily] =
              _FamilyOverrideRef(override, this);
        } else {
