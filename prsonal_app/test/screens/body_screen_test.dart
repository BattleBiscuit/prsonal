...
BodyMetric _m(BodyMetricType t, double v) =>
    BodyMetric(id: '${t.name}1', type: t, value: v, loggedAt: DateTime(2026, 6, 23));

Future<_MockBodyService> _pump(WidgetTester tester,
    {Map<BodyMetricType, BodyMetric?>? latest, List<BodyMetric>? weightHistory}) async {
  final service = _MockBodyService();
  when(() => service.watchLatest())
      .thenAnswer((_) => Stream.value(latest ?? const {}));
  when(() => service.watchHistory(any(), days: any(named: 'days')))
      .thenAnswer((_) => Stream.value(const []));
  when(() => service.watchHistory(BodyMetricType.weight, days: any(named: 'days')))
      .thenAnswer((_) => Stream.value(weightHistory ?? const []));
  when(() => service.log(any(), any(), at: any(named: 'at'))).thenAnswer((_) async {});
...
void main() {
  group('BodyScreen', () {
    testWidgets('AC-001: Renders a metric card for each of the six body metric types',
        (tester) async {
      await _pump(tester);
      expect(find.byType(BodyMetricCard), findsNWidgets(6));
    });

    testWidgets('AC-002: A metric card shows the latest value or a dash when none has been logged',
        (tester) async {
      await _pump(tester, latest: {BodyMetricType.weight: _m(BodyMetricType.weight, 82.5)});
      expect(find.textContaining('82.5'), findsOneWidget);
      expect(find.text('—'), findsWidgets);
    });

    testWidgets('AC-003: Tapping a metric card opens a log sheet for that metric', (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-004: Submitting the log sheet records a new value', (tester) async {
    ...
    testWidgets('AC-005: Renders a history list for metrics that have entries', (tester) async {
      await _pump(tester,
