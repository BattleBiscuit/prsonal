...
Future<String> _seedRoutine(AppDatabase db,
    {int setCount = 2, double plannedWeight = 80, int reps = 8}) async {
  ...
  await db.insertRoutineExerciseRow(RoutineExercisesCompanion.insert(
    id: const Uuid().v4(),
    routineId: routineId,
...
void main() {
  ...
  group('SessionService', () {
    test('AC-001: startSession creates an active session and one workout set per planned set of the routine, with the cursor on the first set',
        () async {
    ...
    test('AC-002: resumeActiveSession restores an existing active session with the cursor at the first incomplete set, and returns false when none exists',
        () async {
      expect(await engine().resumeActiveSession(), isFalse);
      final routineId = await _seedRoutine(db, setCount: 2);
      await engine().startSession(routineId: routineId);
      await engine()
          .markCurrentSetComplete(actualPrimary: 8, actualSecondary: 80, isBodyweight: false);
    ...
    test('AC-003: markCurrentSetComplete records the actual reps and weight, sets completedAt, and freezes effectiveWeight and estimated1RM',
        () async {
      final routineId = await _seedRoutine(db);
      await engine().startSession(routineId: routineId);
      await engine()
          .markCurrentSetComplete(actualPrimary: 8, actualSecondary: 82.5, isBodyweight: false);
      ...
      expect(set.estimated1RM, closeTo(estimatedOneRepMax(effectiveWeight: 82.5, reps: 8), 1e-9));
    });

    test('AC-004: a completed strength set whose estimated 1RM exceeds every prior set of that exercise is flagged isPR',
        () async {
      final routineId = await _seedRoutine(db, setCount: 2, plannedWeight: 80, reps: 8);
      await engine().startSession(routineId: routineId);
      await engine()
          .markCurrentSetComplete(actualPrimary: 8, actualSecondary: 80, isBodyweight: false);
      await engine()
          .markCurrentSetComplete(actualPrimary: 8, actualSecondary: 90, isBodyweight: false);
      final sets = await db.workoutSetsForSession(state().session.id);
      expect(sets[1].isPR, isTrue);
    });

    test('AC-005: a completed strength set whose estimated 1RM does not exceed the prior best is not flagged isPR',
        () async {
      final routineId = await _seedRoutine(db, setCount: 2);
      await engine().startSession(routineId: routineId);
      await engine()
    ...
    test('AC-006: markCurrentSetComplete advances the cursor to the next set', () async {
      final routineId = await _seedRoutine(db, setCount: 2);
      await engine().startSession(routineId: routineId);
      await engine()
          .markCurrentSetComplete(actualPrimary: 8, actualSecondary: 80, isBodyweight: false);
    ...
    test('AC-007: completing a PR set triggers a PR haptic and a non-PR set triggers a tap haptic',
        () async {
      final routineId = await _seedRoutine(db, setCount: 2);
      await engine().startSession(routineId: routineId);
      ...
      await engine()
          .markCurrentSetComplete(actualPrimary: 8, actualSecondary: 50, isBodyweight: false);
      expect(platform.hapticLog.last, 'tap');
    });

    test('AC-008: finishSession marks the session completed and writes the logged actuals back to the routine as new planned targets',
        () async {
      final routineId = await _seedRoutine(db, setCount: 1, plannedWeight: 80, reps: 8);
      await engine().startSession(routineId: routineId);
      await engine()
          .markCurrentSetComplete(actualPrimary: 10, actualSecondary: 85, isBodyweight: false);
    ...
    test('AC-009: abandonSession deletes the session and all of its sets and clears the active state',
        () async {
      final routineId = await _seedRoutine(db);
      await engine().startSession(routineId: routineId);
      final sessionId = state().session.id;
    ...
    test('AC-010: startRest schedules a rest-complete notification and cancelRest cancels it',
        () async {
      final routineId = await _seedRoutine(db);
      await engine().startSession(routineId: routineId);
      engine().startRest(90);
    ...
    test('AC-011: a mutating method called with no active session throws a StateError',
        () async {
      expect(
        () => engine().markCurrentSetComplete(
            actualPrimary: 8, actualSecondary: 80, isBodyweight: false),
        throwsStateError,
      );
    });
  });
