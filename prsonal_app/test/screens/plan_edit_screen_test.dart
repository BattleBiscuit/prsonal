...
RoutineSummary _routine(String id, String name) => RoutineSummary(
    id: id, name: name, exerciseCount: 3, notes: null, updatedAt: DateTime(2026, 6, 1));

PlanDraft _draft() => PlanDraft(id: 'p1', name: 'PPL', entries: [
      PlanEntryDraft(id: 'e1', routineId: 'r1', routineName: 'Push Day A', dayOfWeek: 0),
    ]);

GoRouter _router(Widget screen) => GoRouter(routes: [
      GoRoute(path: '/', name: 'plans', builder: (_, __) => screen),
      GoRoute(path: '/pick', name: 'session-pick', builder: (_, __) => const Text('BACK')),
    ]);

Future<_MockPlansService> _pump(WidgetTester tester, {String? planId, PlanDraft? draft}) async {
  ...
  await tester.pumpWidget(ProviderScope(
    overrides: [
      ...
      routinesListProvider.overrideWith((ref) => Stream.value([_routine('r1', 'Push Day A')])),
      if (planId != null) planDraftProvider(planId).overrideWith((ref) async => draft),
    ],
    child: MaterialApp.router(routerConfig: _router(PlanEditScreen(planId: planId))),
  ));
...
void main() {
  group('PlanEditScreen', () {
    testWidgets('AC-001: In create mode the name field is empty; in edit mode it is populated from the plan',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-002: Tapping "Add" opens a routine picker and selecting a routine adds an entry',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-003: Each entry exposes a day selector to assign its day of the week',
        (tester) async {
      await _pump(tester, planId: 'p1', draft: _draft());
      expect(find.byType(DayOfWeekSelector), findsOneWidget);
    });

    testWidgets('AC-004: Removing an entry removes it from the list', (tester) async {
      await _pump(tester, planId: 'p1', draft: _draft());
      expect(find.byType(DayOfWeekSelector), findsOneWidget);
    ...
    testWidgets('AC-005: Tapping Save persists the plan name and entries and navigates back',
        (tester) async {
    ...
    testWidgets('AC-007: In edit mode, deleting the plan removes it and navigates back',
        (tester) async {
