...
Widget _wrap(Widget child) => MaterialApp(home: Scaffold(body: SizedBox(width: 400, child: child)));
...
void main() {
  group('ChartSlider', () {
    testWidgets('AC-001: Widget renders the first page and its title', (tester) async {
      await tester.pumpWidget(_wrap(_slider()));
      expect(find.text('PAGE ONE'), findsOneWidget);
      expect(find.text('Muscle balance'), findsOneWidget);
    });

    testWidgets('AC-002: Widget renders one pagination dot per page', (tester) async {
      await tester.pumpWidget(_wrap(_slider()));
    ...
    testWidgets('AC-004: Tapping a pagination dot jumps to that page', (tester) async {
      await tester.pumpWidget(_wrap(_slider()));
