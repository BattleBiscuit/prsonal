...
void main() {
  ...
  group('RoutinesService', () {
    test('AC-001: createRoutine inserts a routine and returns its id', () async {
    ...
    test('AC-002: watchRoutines emits routines ordered most-recently-updated first, each with its exercise count',
        () async {
      final a = await service.createRoutine(name: 'A');
      final b = await service.createRoutine(name: 'B');
      final ex = await seedExercise('Bench');
      await service.addExercise(b, exerciseId: ex, sets: [SetTarget.strength()]);
    ...
    test('AC-003: updateRoutine changes the fields and refreshes updatedAt', () async {
    ...
    test('AC-004: deleteRoutine removes the routine and all of its exercises', () async {
      final id = await service.createRoutine(name: 'A');
      final ex = await seedExercise('Bench');
      await service.addExercise(id, exerciseId: ex, sets: [SetTarget.strength()]);
      await service.deleteRoutine(id);
      expect(() => service.getRoutineForEdit(id), throwsA(isA<NotFoundException>()));
    });

    test('AC-005: addExercise appends an exercise at the next position with its planned sets',
        () async {
      final id = await service.createRoutine(name: 'A');
      final ex = await seedExercise('Bench');
      await service.addExercise(id, exerciseId: ex, sets: [SetTarget.strength(), SetTarget.strength()]);
      final draft = await service.getRoutineForEdit(id);
      expect(draft.exercises.single.position, 0);
      expect(draft.exercises.single.sets.length, 2);
    });

    test('AC-006: removeExercise deletes the exercise and renumbers the remaining positions to be contiguous',
        () async {
      final id = await service.createRoutine(name: 'A');
      ...
      for (final e in [e1, e2, e3]) {
        await service.addExercise(id, exerciseId: e, sets: [SetTarget.strength()]);
      }
      final middle = (await service.getRoutineForEdit(id)).exercises[1].id;
      await service.removeExercise(middle);
      final positions =
          (await service.getRoutineForEdit(id)).exercises.map((e) => e.position).toList();
    ...
    test('AC-007: reorderExercises persists the new positions', () async {
      ...
      final e1 = await seedExercise('Bench');
      final e2 = await seedExercise('Squat');
      await service.addExercise(id, exerciseId: e1, sets: [SetTarget.strength()]);
      await service.addExercise(id, exerciseId: e2, sets: [SetTarget.strength()]);
      final ids = (await service.getRoutineForEdit(id)).exercises.map((e) => e.id).toList();
      await service.reorderExercises(id, [ids[1], ids[0]]);
      final reordered = (await service.getRoutineForEdit(id)).exercises.map((e) => e.id).toList();
      expect(reordered, [ids[1], ids[0]]);
    });

    test('AC-008: getRoutineForEdit returns the routine with its exercises resolved by library name in position order',
        () async {
      final id = await service.createRoutine(name: 'A');
