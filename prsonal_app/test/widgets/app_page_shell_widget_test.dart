...
void main() {
  group('AppPageShell', () {
    testWidgets('AC-001: Widget renders the PRsonal brand wordmark', (tester) async {
    ...
    testWidgets('AC-002: Widget renders the header content when provided',
      ...
      await tester.pumpWidget(
        _wrap(const AppPageShell(header: Text('Progress'), child: Text('body'))),
      );
      expect(find.text('Progress'), findsOneWidget);
    });

    testWidgets('AC-003: Widget shows the workout-in-progress banner with the routine name when showWorkoutBanner is true',
        (tester) async {
      await tester.pumpWidget(
        _wrap(const AppPageShell(
    ...
    testWidgets('AC-004: Widget hides the workout-in-progress banner when showWorkoutBanner is false',
        (tester) async {
      await tester.pumpWidget(
        _wrap(const AppPageShell(showWorkoutBanner: false, child: Text('body'))),
      );
      expect(find.text('Workout in progress'), findsNothing);
    });

    testWidgets('AC-005: Widget calls onResumeWorkout when the banner is tapped',
        (tester) async {
      var resumed = false;
      await tester.pumpWidget(
        _wrap(AppPageShell(
