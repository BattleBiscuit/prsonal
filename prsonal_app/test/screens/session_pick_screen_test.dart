...
GoRouter _router() => GoRouter(routes: [
      GoRoute(path: '/', name: 'session-pick', builder: (_, __) => const SessionPickScreen()),
      GoRoute(path: '/session/active', name: 'session-active', builder: (_, __) => const Text('ACTIVE')),
      GoRoute(path: '/routines/new', name: 'routine-create', builder: (_, __) => const Text('NEW ROUTINE')),
      GoRoute(path: '/plans/new', name: 'plan-create', builder: (_, __) => const Text('NEW PLAN')),
      GoRoute(path: '/routines/:id/edit', name: 'routine-edit', builder: (_, __) => const Text('EDIT ROUTINE')),
      GoRoute(path: '/plans/:id/edit', name: 'plan-edit', builder: (_, __) => const Text('EDIT PLAN')),
    ]);
...
const _ppl = PlanView(
  ...
  streak: 3,
  entries: [
    PlanEntryView(entryId: 'e1', routineId: 'r1', routineName: 'Push Day A', dayLabel: 'Mon', doneThisWeek: true),
    PlanEntryView(entryId: 'e2', routineId: 'r2', routineName: 'Pull Day', dayLabel: 'Wed', doneThisWeek: false),
  ],
);
...
Future<void> _pump(
  ...
  List<UnplannedRoutine> unplanned = const [UnplannedRoutine(id: 'r9', name: 'Mobility')],
}) async {
...
void main() {
  group('SessionPickScreen', () {
    testWidgets('AC-001: Renders a plan block for each active plan, showing the plan name and an entry row per entry',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-002: Each plan entry shows its day label, routine name, and a done indicator reflecting whether it was completed this week',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-003: Tapping an entry\'s start button starts a session for that routine (with its plan context) and navigates to session-active',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-004: Renders an "Unplanned" block listing routines not in any active plan',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-005: Tapping the Add FAB opens a sheet offering "New plan" and "New routine"',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-006: Selecting "New routine" navigates to routine-create and "New plan" navigates to plan-create',
        (tester) async {
      await _pump(tester);
      await tester.tap(find.text('Add'));
      await tester.pumpAndSettle();
    ...
    testWidgets('AC-007: Shows an empty state when there are no plans and no routines',
        (tester) async {
      await _pump(tester, plans: const [], unplanned: const []);
      expect(find.text('Nothing here yet'), findsOneWidget);
    });

    testWidgets('AC-008: A plan block shows its streak count when the streak is greater than zero',
        (tester) async {
      await _pump(tester);
