...
RoutineSummary _summary(String id, String name, {int count = 3}) =>
    RoutineSummary(
      id: id,
      name: name,
      exerciseCount: count,
...
GoRouter _router() => GoRouter(
  routes: [
    ...
    GoRoute(
      path: '/routines/new',
      name: 'routine-create',
      builder: (_, __) => const Text('CREATE'),
    ),
    GoRoute(
...
Future<void> _pump(WidgetTester tester, List<RoutineSummary> routines) async {
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        routinesListProvider.overrideWith((ref) => Stream.value(routines)),
      ],
...
void main() {
  group('RoutinesScreen', () {
    testWidgets(
      ...
      (tester) async {
        await _pump(tester, [
          _summary('r1', 'Push Day A'),
          _summary('r2', 'Pull Day'),
        ]);
    ...
    testWidgets('AC-002: Tapping a routine card navigates to routine-edit', (
      tester,
    ) async {
    ...
    testWidgets('AC-003: Tapping "+ New" navigates to routine-create', (
      tester,
    ) async {
      await _pump(tester, [_summary('r1', 'Push Day A')]);
    ...
    testWidgets('AC-005: Shows an empty state when there are no routines', (
      tester,
    ) async {
