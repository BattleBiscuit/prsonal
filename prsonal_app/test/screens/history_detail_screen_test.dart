...
SessionDetail _detail() => SessionDetail(
      ...
      exercises: const [
        DetailExercise(name: 'Bench Press', sets: [
          SetTableRow(
              setIndex: 0,
              plannedLabel: '8×80kg',
...
Future<_MockHistoryService> _pump(WidgetTester tester) async {
  ...
  when(() => service.updateSetActuals(any(),
      reps: any(named: 'reps'),
      weight: any(named: 'weight'),
      isBodyweight: any(named: 'isBodyweight'),
      skipped: any(named: 'skipped'))).thenAnswer((_) async {});
...
void main() {
  group('HistoryDetailScreen', () {
    testWidgets('AC-001: Shows the workout summary (routine, date, duration, volume, status)',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-002: Shows a per-exercise set table of planned and actual values',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-003: Shows a PR banner listing exercises that set a personal record',
        (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-004: Tapping Edit makes the actual values editable', (tester) async {
      await _pump(tester);
    ...
    testWidgets('AC-005: Saving edits persists the changes and exits edit mode', (tester) async {
      ...
      await tester.pumpAndSettle();
      verify(() => service.updateSetActuals(any(),
          reps: any(named: 'reps'),
          weight: any(named: 'weight'),
          isBodyweight: any(named: 'isBodyweight'),
          skipped: any(named: 'skipped'))).called(greaterThan(0));
